...
bool quickMathGame() {
    ...
    int b = rand() % 10 + 1;
    int answer;
    cout << "Quick Math Challenge! What's " << a << " + " << b << "? ";
    cin >> answer;
    return answer == (a + b);
}

bool logicPuzzleGame() {
    cout << "Logic Puzzle: What comes next in the sequence?\n2, 4, 8, 16, ?\n";
    int answer;
    cin >> answer;
    return answer == 32;
}

bool vaultCrackingGame() {
    srand(time(0));
    int code = rand() % 10 + 1;
    int guess;
    cout << "Vault Cracking! Guess the number between 1-10: ";
    cin >> guess;
    return guess == code;
}
...
void Acts::Act3_day1(Player &player) {
    ...
    cout << "Evening falls. You play a puzzle game to relax.\n";
    if (logicPuzzleGame()) {
        cout << "You feel sharper. Intelligence +1\n";
        player.Intelligence += 1;
    } else {
        cout << "You feel confused but entertained.\n";
    }
    pressEnter();
...
void Acts::Act3_day2(Player &player) {
    ...
    cout << "You warm up with a challenge.\n";
    if (quickMathGame()) {
        cout << "Your mind feels agile. Wisdom +1\n";
        player.Wisdom += 1;
    } else {
        cout << "Your timing is off today.\n";
    }
    pressEnter();
...
void Acts::Act3_day3(Player &player) {
    ...
    if (vaultCrackingGame() || (player.Intelligence + player.Dexterity >= 6) || (hasGear && alliesMet)) {
